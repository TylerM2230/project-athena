...
router.post('/', async (req, res) => {
    try {
        const { title, description, category = 'personal', targetFrequency = 'daily', targetCount = 1, icon, reminderTime, goalId } = req.body;
        if (!title) {
...
router.put('/:id', async (req, res) => {
    try {
        ...
        if (targetFrequency !== undefined)
            updates.targetFrequency = targetFrequency;
...
router.get('/:id/entries', async (req, res) => {
    try {
        ...
        const entries = await db.getHabitEntries(req.params.id, startDate, endDate);
        res.json(entries);
    }
    catch (error) {
